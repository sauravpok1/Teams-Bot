...
const questionVisitor = async function (question, ui, inputs, step, totalSteps) {
    if (inputs[question.name] !== undefined) {
        return neverthrow_1.ok({ type: "success", result: inputs[question.name] });
    }
    ...
    return neverthrow_1.err(error_1.returnUserError(new Error(`Unsupported question node type:${JSON.stringify(question)}`), "API", "UnsupportedNodeType"));
};
