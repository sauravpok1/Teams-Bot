"use strict";
// Copyright (c) Microsoft Corporation.
// Licensed under the MIT license.
Object.defineProperty(exports, "__esModule", { value: true });
exports.TokenProvider = exports.TokenAudience = void 0;
const results_1 = require("../results");
const errors_1 = require("../errors");
var TokenAudience;
(function (TokenAudience) {
    TokenAudience["Graph"] = "graph";
    TokenAudience["AppStudio"] = "appStudio";
})(TokenAudience = exports.TokenAudience || (exports.TokenAudience = {}));
class GraphInstance {
    async getToken(ctx) {
        var _a;
        const token = await ((_a = ctx.graphTokenProvider) === null || _a === void 0 ? void 0 : _a.getAccessToken());
        return token;
    }
    async getTenant(ctx) {
        var _a;
        const tokenObject = await ((_a = ctx.graphTokenProvider) === null || _a === void 0 ? void 0 : _a.getJsonObject());
        if (!tokenObject) {
            return undefined;
        }
        const tenantId = tokenObject.tid;
        return tenantId;
    }
}
class AppStudioInstance {
    async getToken(ctx) {
        var _a;
        const token = await ((_a = ctx.appStudioToken) === null || _a === void 0 ? void 0 : _a.getAccessToken());
        return token;
    }
    async getTenant(ctx) {
        var _a;
        const tokenObject = await ((_a = ctx.appStudioToken) === null || _a === void 0 ? void 0 : _a.getJsonObject());
        if (!tokenObject) {
            return undefined;
        }
        const tenantId = tokenObject.tid;
        return tenantId;
    }
}
class TokenProvider {
    static async init(ctx, audience = TokenAudience.Graph) {
        this.audience = audience;
        let instance;
        if (audience === TokenAudience.AppStudio) {
            instance = new AppStudioInstance();
        }
        else {
            instance = new GraphInstance();
        }
        const token = await instance.getToken(ctx);
        if (token) {
            TokenProvider.token = token;
        }
        else {
            throw results_1.ResultFactory.SystemError(errors_1.GetTokenError.name, errors_1.GetTokenError.message(this.audience));
        }
        const tenantId = await instance.getTenant(ctx);
        if (tenantId) {
            this.tenantId = tenantId;
        }
        else {
            throw results_1.ResultFactory.SystemError(errors_1.TenantNotExistError.name, errors_1.TenantNotExistError.message());
        }
    }
}
exports.TokenProvider = TokenProvider;
//# sourceMappingURL=tokenProvider.js.map