...
function sqlUserNameValidator(name) {
    ...
    if (!name) {
        return message_1.Message.inputCheck.sqlUserNameEmpty;
    }
    if (invalidSqlUserName.names.includes(name.toLowerCase())) {
        return message_1.Message.inputCheck.sqlUserNameContainsSqlIdentifier;
    }
    if (name.search(/[^a-zA-Z0-9]+/) >= 0) {
        return message_1.Message.inputCheck.sqlUserNameContainsNonAlphanumeric;
    }
    if (name.match(/^[0-9]/)) {
        return message_1.Message.inputCheck.sqlUserNameStartWithNumber;
    }
    return undefined;
}
...
function sqlPasswordValidatorGenerator(name) {
    return (password) => {
        ...
        if (!password) {
            return message_1.Message.inputCheck.sqlPasswordEmpty;
        }
        if (password.length < 8) {
            return message_1.Message.inputCheck.sqlPasswordLengthLessThan8;
        }
        if (password.length > 128) {
            return message_1.Message.inputCheck.sqlPasswordLengthGreatThan128;
        }
        ...
        if (containLowerLetters + containUpperLetters + containNumbers + containSpecialCharacters < 3) {
            return message_1.Message.inputCheck.sqlPasswordMustContain3Categories;
        }
        if (password.toLowerCase().search(name.toLowerCase()) >= 0) {
            return message_1.Message.inputCheck.sqlPasswordCannotContainUserName;
        }
        return undefined;
    };
}
...
function sqlConfirmPasswordValidatorGenerator(password) {
    return (confirm) => {
        if (password !== confirm) {
            return message_1.Message.inputCheck.sqlPasswordMustMatch;
        }
        return undefined;
    };
}
