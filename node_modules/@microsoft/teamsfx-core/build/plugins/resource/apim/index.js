...
let ApimPlugin = class ApimPlugin {
    ...
    async getQuestionsForUserTask(func, ctx) {
        return await this.executeWithFxError(constants_1.PluginLifeCycle.GetQuestionsForUserTask, _getQuestionsForUserTask, ctx, func);
    }
    ...
    async executeWithFxError(lifeCycle, fn, ctx, ...params) {
        var _a;
        try {
            await this.progressBar.init(constants_1.PluginLifeCycleToProgressStep[lifeCycle], ctx);
            telemetry_1.Telemetry.sendLifeCycleEvent(ctx.telemetryReporter, ctx.envInfo, lifeCycle, constants_1.OperationStatus.Started);
            const result = await fn(ctx, this.progressBar, ...params);
            telemetry_1.Telemetry.sendLifeCycleEvent(ctx.telemetryReporter, ctx.envInfo, lifeCycle, constants_1.OperationStatus.Succeeded);
            ...
            telemetry_1.Telemetry.sendLifeCycleEvent(ctx.telemetryReporter, ctx.envInfo, lifeCycle, constants_1.OperationStatus.Failed, packagedError);
...
async function _getQuestions(ctx, progressBar, stage) {
    ...
    const questionManager = await factory_1.Factory.buildQuestionManager(ctx);
    switch (stage) {
...
async function _getQuestionsForUserTask(ctx, progressBar, func) {
    const apimConfig = new config_1.ApimPluginConfig(ctx.config, ctx.envInfo.envName);
    const questionManager = await factory_1.Factory.buildQuestionManager(ctx);
...
async function _scaffold(ctx, progressBar) {
    ...
    const scaffoldManager = await factory_1.Factory.buildScaffoldManager(ctx);
    ...
    if (answer.validate) {
        await answer.validate(constants_1.PluginLifeCycle.Scaffold, apimConfig, ctx.root);
    }
...
async function _provision(ctx, progressBar) {
    ...
    const apimConfig = new config_1.ApimPluginConfig(ctx.config, ctx.envInfo.envName);
    const apimManager = await factory_1.Factory.buildApimManager(ctx);
    const aadManager = await factory_1.Factory.buildAadManager(ctx);
    const appName = error_1.AssertNotEmpty("projectSettings.appName", (_a = ctx === null || ctx === void 0 ? void 0 : ctx.projectSettings) === null || _a === void 0 ? void 0 : _a.appName);
...
async function _postProvision(ctx, progressBar) {
    ...
    const apimManager = await factory_1.Factory.buildApimManager(ctx);
    const aadManager = await factory_1.Factory.buildAadManager(ctx);
    const teamsAppAadManager = await factory_1.Factory.buildTeamsAppAadManager(ctx);
    const appName = error_1.AssertNotEmpty("projectSettings.appName", (_a = ctx === null || ctx === void 0 ? void 0 : ctx.projectSettings) === null || _a === void 0 ? void 0 : _a.appName);
    ...
    await progressBar.next(constants_1.ProgressStep.PostProvision, constants_1.ProgressMessages[constants_1.ProgressStep.PostProvision].ConfigApim);
