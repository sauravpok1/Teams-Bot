...
const tools_2 = require("../../core/tools");
...
function convert2PluginContext(pluginName, ctx, inputs, ignoreEmptyProjectPath = false) {
    if (!ignoreEmptyProjectPath && !inputs.projectPath)
        throw error_1.NoProjectOpenedError();
    const envInfo = tools_2.newEnvInfo(inputs.env);
...
async function scaffoldSourceCodeAdapter(ctx, inputs, plugin) {
    ...
    const localSettingsProvider = new __1.LocalSettingsProvider(pluginContext.root);
...
async function provisionResourceAdapter(ctx, inputs, envInfo, tokenProvider, plugin) {
    ...
    return teamsfx_api_1.ok(legacyConfig2EnvState(pluginContext.config, plugin.name));
}
...
function flattenConfigMap(configMap) {
    const map = new teamsfx_api_1.ConfigMap();
    for (const [k, v] of configMap.entries()) {
        if (v instanceof teamsfx_api_1.ConfigMap) {
...
function legacyConfig2EnvState(config, pluginName) {
    ...
    for (const key of Object.keys(output)) {
        if (tools_1.CryptoDataMatchers.has(`${pluginName}.${key}`)) {
...
async function provisionLocalResourceAdapter(ctx, inputs, localSettings, tokenProvider, plugin) {
...
async function configureLocalResourceAdapter(ctx, inputs, localSettings, tokenProvider, plugin) {
...
function setEnvInfoV1ByStateV2(pluginName, pluginContext, envInfoV2) {
    const envInfo = tools_2.newEnvInfo();
