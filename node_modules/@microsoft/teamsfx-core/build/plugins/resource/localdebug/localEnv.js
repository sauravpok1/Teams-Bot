...
class LocalEnvProvider {
    ...
    async loadLocalEnv(includeFrontend, includeBackend, includeBot, includeAuth, isMigrateFromV1) {
        if (await fs.pathExists(this.localEnvFilePath)) {
            return dotenv.parse(await fs.readFile(this.localEnvFilePath));
        }
        else {
            return this.initialLocalEnvs(includeFrontend, includeBackend, includeBot, includeAuth, isMigrateFromV1);
        }
    }
    ...
    initialLocalEnvs(includeFrontend, includeBackend, includeBot, includeAuth, isMigrateFromV1) {
        const localEnvs = {};
        let keys;
        if (includeFrontend) {
            if (includeAuth) {
        ...
        if (includeBot) {
            keys = isMigrateFromV1
                ? Object.values(constants_1.LocalEnvBotKeysMigratedFromV1)
                : Object.values(constants_1.LocalEnvBotKeys);
            for (const key of keys) {
                // initial with empty string
                localEnvs[key] = "";
            }
