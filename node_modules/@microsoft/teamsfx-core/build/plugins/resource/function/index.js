...
let FunctionPlugin = class FunctionPlugin {
    ...
    async preProvision(ctx) {
        this.setContext(ctx);
        const res = await this.runWithErrorWrapper(ctx, enums_1.FunctionEvent.preProvision, () => this.functionPluginImpl.preProvision(ctx));
        return res;
    }
    async provision(ctx) {
        this.setContext(ctx);
        ...
        try {
            ...
            if (e instanceof errors_1.FunctionPluginError) {
                const res = e.errorType === errors_1.ErrorType.User
                    ? result_1.FunctionPluginResultFactory.UserError(e.getMessage(), e.code, undefined, e, e.stack)
                    : result_1.FunctionPluginResultFactory.SystemError(e.getMessage(), e.code, undefined, e, e.stack);
                sendTelemetry && telemetry_helper_1.TelemetryHelper.sendResultEvent(event, res);
                return res;
            }
            ...
            sendTelemetry &&
                telemetry_helper_1.TelemetryHelper.sendResultEvent(event, result_1.FunctionPluginResultFactory.SystemError("Got an unhandled error", UnhandledErrorCode));
            return result_1.FunctionPluginResultFactory.SystemError(e.message, UnhandledErrorCode, undefined, e, e.stack);
        }
    }
};
