...
function generateTasks(includeFrontend, includeBackend, includeBot, includeAuth, isMigrateFromV1, programmingLanguage) {
    /**
     ...
     *   - bot npm install
     */
    const tasks = [preDebugCheck(includeBot, isMigrateFromV1)];
    if (!isMigrateFromV1) {
...
function preDebugCheck(includeBot, isMigrateFromV1) {
    return {
...
function startNgrok() {
    return {
        label: "start ngrok",
        type: teamsfx_api_1.ProductName,
        command: "ngrok start",
        isBackground: true,
