...
async function executeConcurrently(step, lifecycleAndContext) {
    ...
    const promises = lifecycleAndContext.map(async (pair) => {
        ...
        if (lifecycle) {
            try {
                ...
                return teamsfx_api_1.err(new teamsfx_api_1.SystemError("UnknownError", `[Solution.executeConcurrently part 1] unknown error from plugin: ${pluginName}, taskName:${taskname}, error: ${JSON.stringify(e)}`, "Solution"));
            }
        }
        else {
            return teamsfx_api_1.ok(undefined);
    ...
    try {
        ...
        throw new teamsfx_api_1.SystemError("UnknownError", `[Solution.executeConcurrently part 2] unknown error: ${JSON.stringify(e)}`, "Solution");
    }
}
