...
function getActivatedResourcePlugins(solutionSettings) {
    ...
    if (activatedPlugins.length === 0) {
        throw teamsfx_api_1.returnUserError(new Error(`No plugin selected`), constants_1.SolutionSource, constants_1.SolutionError.NoResourcePluginSelected);
    }
 ...
 * @returns activated resource plugins
 */
function getActivatedV2ResourcePlugins(solutionSettings) {
    const activatedPlugins = getAllV2ResourcePlugins().filter((p) => p.activate && p.activate(solutionSettings) === true);
    if (activatedPlugins.length === 0) {
        throw teamsfx_api_1.returnUserError(new Error(`No plugin selected`), constants_1.SolutionSource, constants_1.SolutionError.NoResourcePluginSelected);
    }
    return activatedPlugins;
